...
class SchemaRegistryModel(BaseDataModel):
    ...
    async def get_or_register(
        ...
        Unregistered sheet -> inserted on the spot with no mandatory
        fields. Every synced sheet is Bucket A — OneDrive sync is
        strictly Bucket A now, so there is no bucket to classify.
        Already-registered sheet -> its mandatory_fields are left
        untouched (that's the one deliberate, reviewed step an engineer
        takes later, via `set_mandatory_fields`), but its column list is
