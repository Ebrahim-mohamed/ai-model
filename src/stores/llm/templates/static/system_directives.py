...
whatsapp_mode_a_reply_directive = Template("\n".join([
    ...
    "متاحة (وأبدًا العكس لو مكتوب إنها متاحة).",
    "",
    ...
    "'Services' أو 'Branches') ترجمها للعربي.",
    "",
    "3. الشخصية والأسلوب: اتكلمي بعامية مصرية طبيعية وصافية 100% — من "
    "غير فصحى رسمية جامدة، ومن غير أي لهجة خليجية.",
    "",
    ...
    "",
    "5. سؤال المتابعة: اختم ردك بسؤال متابعة واحد بس، مرتبط بموضوع الرد.",
    "",
...
whatsapp_breadth_classification_guidance = Template("\n".join([
    "Domain-specific narrow-vs-broad examples for this business:",
    "- broad: a question asking what exists/is offered in general, with "
    "no single specific test/exam/service named — e.g. 'عندكم أشعة إيه؟', "
    "'ما هي الخدمات المتاحة؟', 'إيه أنواع التحاليل عندكم؟'.",
    "- narrow: a question naming one specific test, exam, branch, or "
    "asking one specific yes/no fact about one thing — e.g. 'فيه أسانسير "
    "في فرع المهندسين؟', 'سعر تحليل السكر كام؟', 'محتاج تحضير إيه لأشعة "
    "الـCT؟'.",
    "When in doubt between the two, prefer 'broad' only if the message "
    "genuinely names no specific test/exam/branch at all — a message "
    "naming one specific thing is 'narrow' even if phrased generally "
    "(e.g. 'أشعة الـCT عندكم إيه تحضيراتها؟' is narrow: one named exam).",
]))
...
whatsapp_out_of_domain_directive = Template("\n".join([
    ...
    "والتحاليل الطبية. المريض سأل عن حاجة إحنا مش بنقدمها، ومفيش عندك "
    "أي معلومة عنها (مفيش CONTEXT اتبعتلك عشان كده بالظبط).",
